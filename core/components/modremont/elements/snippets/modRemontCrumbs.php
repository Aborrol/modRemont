...
/** @var modX $modx */
// форк pdoCrumbs
// Добавлен параметр в вызов extra
// Который принимает массив для расширения хлебных крошек
// В формате [['uri'=>'pagetitle'],['uri'=>'pagetitle']]
// Если елемент только один, то допустим формат
// ['uri'=>'pagetitle']
// Пример вызова в чанке modRemontCrumbs
...
$rows = $pdoFetch->run();
$baseurl = $pdoFetch->makeUrl($rows[0]['id'], $rows[0]);
if(is_array($extra)){
    foreach($extra as $ut => $el){
        if(is_array($el)){
            foreach($el as $u => $t){
                array_unshift($rows, ['pagetitle' => $t, 'menutitle' => $t, 'uri' => $u, 'class_key' => 'modRemontDocument' ]);
            }
        }
        else{
            array_unshift($rows, ['pagetitle' => $el, 'menutitle' => $el, 'uri' => $ut, 'class_key' => 'modRemontDocument' ]);
        }
    }
    $rows[0]['last'] = 1;
}
    ...
    foreach ($rows as $row) {
        if (!empty($useWeblinkUrl) && $row['class_key'] == 'modWebLink') {
            $row['link'] = is_numeric(trim($row['content'], '[]~ '))
                ...
                : $row['content'];
        } elseif ($row['class_key'] == 'modRemontDocument') {
            $row['link'] = $baseurl.$row['uri'].'/';
        } else {
        ...
        if ($row['id'] == $resource->id && empty($showCurrent)) {
            continue;
        } elseif (($row['id'] == $resource->id && !empty($tplCurrent) && !count($extra)>0) || $row['last']) {
            $tpl = $tplCurrent;
