modRemont.grid.Default = function (config) {
  config = config || {};
  ...
  Ext.applyIf(config, {
      ...
      baseParams: {},
      cls: config['cls'] || 'main-wrapper modremont-grid',
      autoHeight: true,
      paging: true,
      remoteSort: true,
      ...
      viewConfig: {
          ...
          getRowClass: function (rec) {
              ...
              if (rec.data['published'] != undefined && rec.data['published'] == 0) {
                  cls.push('modremont-row-unpublished');
              }
              if (rec.data['active'] != undefined && rec.data['active'] == 0) {
                  cls.push('modremont-row-inactive');
              }
              if (rec.data['deleted'] != undefined && rec.data['deleted'] == 1) {
                  cls.push('modremont-row-deleted');
              }
              if (rec.data['required'] != undefined && rec.data['required'] == 1) {
                  cls.push('modremont-row-required');
              }
              return cls.join(' ');
          }
      },
  });
  modRemont.grid.Default.superclass.constructor.call(this, config);
...
Ext.extend(modRemont.grid.Default, MODx.grid.Grid, {
...
Ext.reg('modremont-grid-default', modRemont.grid.Default);