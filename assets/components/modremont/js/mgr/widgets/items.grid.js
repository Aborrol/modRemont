modRemont.grid.Items = function (config) {
    config = config || {};
    if (!config.id) {
        config.id = 'modremont-grid-items';
    }
    Ext.applyIf(config, {
    ...
    modRemont.grid.Items.superclass.constructor.call(this, config);
...
Ext.extend(modRemont.grid.Items, MODx.grid.Grid, {
    windows: {},
    ...
    createItem: function (btn, e) {
        var w = MODx.load({
            xtype: 'modremont-item-window-create',
            id: Ext.id(),
            listeners: {
                success: {
                    fn: function () {
        ...
        MODx.Ajax.request({
            ...
            listeners: {
                success: {
                    fn: function (r) {
                        var w = MODx.load({
                            xtype: 'modremont-item-window-update',
                            id: Ext.id(),
                            record: r,
                            listeners: {
                                success: {
    ...
    removeItem: function () {
        ...
        MODx.msg.confirm({
            title: ids.length > 1
                ? _('modremont_items_remove')
                : _('modremont_item_remove'),
            text: ids.length > 1
                ? _('modremont_items_remove_confirm')
                : _('modremont_item_remove_confirm'),
            url: this.config.url,
            params: {
    ...
    getColumns: function () {
        return [{
            header: _('modremont_item_id'),
            dataIndex: 'id',
            sortable: true,
            width: 70
        }, {
            header: _('modremont_item_image'),
            dataIndex: 'image',
            sortable: true,
            width: 100,
            ...
            header: _('modremont_item_name'),
            dataIndex: 'name',
            sortable: true,
            width: 150,
        }, {
            header: _('modremont_item_database'),
            dataIndex: 'supports_db',
            sortable: false,
            width: 150,
        }, {
            header: _('modremont_item_description'),
            dataIndex: 'description',
            sortable: false,
            width: 250,
        }, {
            header: _('modremont_item_category'),
            dataIndex: 'category_name',
            sortable: false,
            width: 150,
        }, {
            header: _('modremont_products'),
            dataIndex: 'productsTitle',
            sortable: false,
            width: 150,
        }, {
            header: _('modremont_item_user'),
            dataIndex: 'createdby_name',
            sortable: false,
            width: 100,
        }, {
            header: _('modremont_item_createdon'),
            dataIndex: 'createdon',
            sortable: true,
            width: 100,
            renderer: modRemont.utils.renderDateTime,
        }, {
            header: _('modremont_item_active'),
            dataIndex: 'active',
    ...
    getTopBar: function () {
        return [{
            xtype: 'button',
            cls: 'primary-button',
            text: '<i class="icon icon-download"></i> ' + _('modremont_item_upload'),
            handler: this.uploadItem,
            scope: this
        }, {
            text: '<i class="icon icon-cogs"></i> ',
            menu: [{
                text: '<i class="icon icon-plus"></i> ' + _('modremont_item_create'),
                cls: 'modremont-cogs',
                handler: this.createItem,
                scope: this
            }, {
                text: '<i class="icon icon-trash-o red"></i> ' + _('modremont_item_remove'),
                cls: 'modremont-cogs',
                handler: this.removeItem,
                scope: this
            }, {
                text: '<i class="icon icon-toggle-on green"></i> ' + _('modremont_item_enable'),
                cls: 'modremont-cogs',
                handler: this.enableItem,
                scope: this
            }, {
                text: '<i class="icon icon-toggle-off red"></i> ' + _('modremont_item_disable'),
                cls: 'modremont-cogs',
...
Ext.reg('modremont-grid-items', modRemont.grid.Items);
